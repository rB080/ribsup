...
class Generator_v4(nn.Module):
    def __init__(self, attention=True, bilinear=False):
...
class Generator_v4_no_attn(nn.Module):
    def __init__(self, bilinear=False):
        super(Generator_v4_no_attn, self).__init__()
        self.bilinear = bilinear
        factor = 2 if bilinear else 1
...
def get_model_set(args):
    if args.use_attention: Gxy, Gyx, Dx, Dy = Generator_v4(attention=args.use_attention), Generator_v4(attention=args.use_attention), Discriminator(), Discriminator()
    else: Gxy, Gyx, Dx, Dy = Generator_v4_no_attn(), Generator_v4_no_attn(), Discriminator(), Discriminator()
